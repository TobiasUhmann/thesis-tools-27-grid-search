    ...
    for dataset, sent_count in dataset_choices:
        for model in ['base', 'ower']:
            for mode in ['mean', 'max', 'sum']:
                ...
                args.log_dir = f'runs/pooling/{dataset}_{model}_{mode}'
                args.mode = mode
                args.model = model
                args.save_dir = f'models/pooling/{dataset}_{model}_{mode}'

                logging.info(f'Training on dataset {dataset} using model {model} with mode {mode}')
                train(args)
